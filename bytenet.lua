--[[
    ...
    Notes:
    i) Change ByteNet and Packets path
    ii) To run remotes do this format:

        require(path.to.packetsmodule).packetname.send({
        foo = 1
        })

        e.g.:
        
        local Module = require(game.ReplicatedStorage.Modules.Packets)
        Module.ToolSwap.send({
            ["key1"] = 3,
            ["key2"] = 2
        })
...
local Packets = require(game.ReplicatedStorage.Modules.Packets) -- path to packets module
